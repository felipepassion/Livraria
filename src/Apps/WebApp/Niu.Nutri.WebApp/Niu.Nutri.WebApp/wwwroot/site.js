var GLOBAL = {};
GLOBAL.DotNetMapReference = null;

window.SetDotNetMapReference = function (pDotNetReference) {
    GLOBAL.DotNetMapReference = pDotNetReference;
};
...
function getNextInputToken(elementId) {
    document.querySelector(`#input${elementId}`).focus();
};
...
window.moveModalToTop = (modalId) => {
    var test = $("#modal_" + modalId);
    test.appendTo('main');
}
...
function submit(formId) {
    $("#modal_" + formId).find(".submit").click();
}
