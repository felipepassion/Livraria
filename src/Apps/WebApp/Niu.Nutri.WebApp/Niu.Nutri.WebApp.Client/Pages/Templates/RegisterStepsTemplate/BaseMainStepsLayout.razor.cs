...
namespace Niu.Nutri.Shared.Blazor.Components.Templates.RegisterStepsTemplate
{
    ...
    {
        [Parameter] public required IValidator TValidator { get; set; }
        ...
        [Parameter] public EventCallback<ModelType> ModelChanged { get; set; }
        ...
        [Parameter] public required IBaseFormPagina Modal { get; set; } = new CadastroModal();
        ...
        public WarningModal ErrorModal { get; set; } = default!;
        ...
        protected async Task SubmitForm() => await OnSubmitForm(this.EditContext);
        ...
        protected async Task OnSubmitForm(EditContext editContext)
        {
            ...
            if (result.StatusCode == System.Net.HttpStatusCode.OK)
            {
                ...
                Model = result.Data?.Data as ModelType
                    ?? await _client.OnInitializedAsync<ModelType>(Model.ExternalId ?? throw new NullReferenceException(Model.ExternalId))
                        ?? throw new NullReferenceException("Error to refresh model after creation");
            ...
            this.ModalContext.IsSubmiting = false;
            this.ModalContext.IsAutoSaving = false;
            (Modal as CadastroModal)?.NextAndBackButtonsContainer?.RefreshMe();
            await InvokeAsync(this.StateHasChanged);
        }
            ...
            //try
            {
                ...
                else
                {
                    this.Context ??= new StepsContext
                        (this.Id, step, Model.MaxSteps, Model, Backward, ChangeStep, async () => { OnKeyboardDown(); },
                         this.PaginationLayoutRefresh, this.SubmitForm, this.Modal?.OnFormSubmited,  this.RenderRegisterForm);
        ...
        async Task Finish()
        {
            ...
            if (this.Modal.OnFormSubmited != null)
                await this.Modal.OnFormSubmited(this.EditContext);
        }
        ...
        private RenderFragment AddContent(object Model, Type type)
            => builder =>
            {
                ...
                builder.AddAttribute(3, "StepsContext", Context);
                builder.AddAttribute(4, "CadastroModalContext", Modal?.ModalContext ?? new());
                builder.AddAttribute(5, "layout", layout);
                //builder.AddAttribute(3, "Context", Con    text);
        ...
        [CascadingParameter] ModalContext ModalContext { get; set; } = new();
