...
namespace Niu.Nutri.SystemSettings.Domain.Aggregates.SystemSettingsAgg.Filters{
	using Entities;
	...
	public static class SystemPanelSubItemFilters 
	{
		...
		public static Specification<SystemPanelSubItem> GetFiltersSpecification(this SystemPanelSubItemQueryModel request, bool isOrSpecification = false) 
		{
			...
			if(request is not null)
			{
				...
				if (request.PositionEqual.HasValue)
				{
					if (isOrSpecification)
						filter |= SystemPanelSubItemSpecifications.PositionEqual(request.PositionEqual.Value);
					else
						filter &= SystemPanelSubItemSpecifications.PositionEqual(request.PositionEqual.Value);
				}
				if (request.PositionNotEqual.HasValue)
				{
					if (isOrSpecification)
						filter |= SystemPanelSubItemSpecifications.PositionNotEqual(request.PositionNotEqual.Value);
					else
						filter &= SystemPanelSubItemSpecifications.PositionNotEqual(request.PositionNotEqual.Value);
				}
				if (request.PositionContains != null)
				{
					if (isOrSpecification)
						filter |= SystemPanelSubItemSpecifications.PositionContains(request.PositionContains);
					else
						filter &= SystemPanelSubItemSpecifications.PositionContains(request.PositionContains);
				}
				if (request.PositionNotContains != null)
				{
					if (isOrSpecification)
						filter |= SystemPanelSubItemSpecifications.PositionNotContains(request.PositionNotContains);
					else
						filter &= SystemPanelSubItemSpecifications.PositionNotContains(request.PositionNotContains);
				}
				if (request.PositionSince.HasValue)
				{
					if (isOrSpecification)
						filter |= SystemPanelSubItemSpecifications.PositionGreaterThanOrEqual(request.PositionSince.Value);
					else
						filter &= SystemPanelSubItemSpecifications.PositionGreaterThanOrEqual(request.PositionSince.Value);
				}
				if (request.PositionUntil.HasValue)
				{
					if (isOrSpecification)
						filter |= SystemPanelSubItemSpecifications.PositionLessThan(request.PositionUntil.Value);
					else
						filter &= SystemPanelSubItemSpecifications.PositionLessThan(request.PositionUntil.Value);
				}
				if (request.PositionNotEqual.HasValue) 
				...
				if (request.PositionLessThan.HasValue) 
				{
					if (isOrSpecification)
						filter |= SystemPanelSubItemSpecifications.PositionLessThan(request.PositionLessThan.Value);
					else
						filter &= SystemPanelSubItemSpecifications.PositionLessThan(request.PositionLessThan.Value);
				}
				if (request.PositionGreaterThan.HasValue) 
				{
					if (isOrSpecification)
						filter |= SystemPanelSubItemSpecifications.PositionGreaterThan(request.PositionGreaterThan.Value);
					else
						filter &= SystemPanelSubItemSpecifications.PositionGreaterThan(request.PositionGreaterThan.Value);
				}
				if (request.PositionLessThanOrEqual.HasValue) 
				{
					if (isOrSpecification)
						filter |= SystemPanelSubItemSpecifications.PositionLessThanOrEqual(request.PositionLessThanOrEqual.Value);
					else
						filter &= SystemPanelSubItemSpecifications.PositionLessThanOrEqual(request.PositionLessThanOrEqual.Value);
				}
				if (request.PositionGreaterThanOrEqual.HasValue) 
				{
					if (isOrSpecification)
						filter |= SystemPanelSubItemSpecifications.PositionGreaterThanOrEqual(request.PositionGreaterThanOrEqual.Value);
					else
						filter &= SystemPanelSubItemSpecifications.PositionGreaterThanOrEqual(request.PositionGreaterThanOrEqual.Value);
				}
	...
	public static class SystemPanelFilters 
	{
		...
		public static Specification<SystemPanel> GetFiltersSpecification(this SystemPanelQueryModel request, bool isOrSpecification = false) 
		{
			...
			if(request is not null)
			{
				...
				{
					if (isOrSpecification)
						filter |= SystemPanelSpecifications.CodeEqual(request.CodeEqual);
					else
						filter &= SystemPanelSpecifications.CodeEqual(request.CodeEqual);
				}
				...
				{
					if (isOrSpecification)
						filter |= SystemPanelSpecifications.CodeNotEqual(request.CodeNotEqual);
					else
						filter &= SystemPanelSpecifications.CodeNotEqual(request.CodeNotEqual);
				}
				if (!string.IsNullOrWhiteSpace(request.CodeNotEqual)) 
				...
				{
					if (isOrSpecification)
						filter |= SystemPanelSpecifications.CodeContains(request.CodeContains);
					else
						filter &= SystemPanelSpecifications.CodeContains(request.CodeContains);
				}
				...
				{
					if (isOrSpecification)
						filter |= SystemPanelSpecifications.CodeStartsWith(request.CodeStartsWith);
					else
						filter &= SystemPanelSpecifications.CodeStartsWith(request.CodeStartsWith);
				}
				if (request.CurrentStepEqual.HasValue)
	...
	public static class SystemPanelGroupFilters 
	{
		...
		public static Specification<SystemPanelGroup> GetFiltersSpecification(this SystemPanelGroupQueryModel request, bool isOrSpecification = false) 
		{
			...
			if(request is not null)
			{
				...
				if (request.IsPrivateEqual.HasValue) 
				{
					if (isOrSpecification)
						filter |= SystemPanelGroupSpecifications.IsPrivateEqual(request.IsPrivateEqual.Value);
					else
						filter &= SystemPanelGroupSpecifications.IsPrivateEqual(request.IsPrivateEqual.Value);
				}
				if (request.CurrentStepEqual.HasValue)
