...
namespace Niu.Nutri.SystemSettings.Domain.Aggregates.SystemSettingsAgg.Profiles
{
	...
	public partial class SystemPanelListiningProfile : Profile
	{
		...
		{
			 CreateMap<SystemPanel, SystemPanelListiningDTO>().ForMember(x=>x.Icon, opt => opt.MapFrom(x=>x.Icon)).ForMember(x=>x.Description, opt => opt.MapFrom(x=>x.Description)).ForMember(x=>x.Code, opt => opt.MapFrom(x=>x.Code));
		}
	}
	public partial class SystemPanelGroupListiningProfile : Profile
	{
		...
		{
			 CreateMap<SystemPanelGroup, SystemPanelGroupListiningDTO>().ForMember(x=>x.Description, opt => opt.MapFrom(x=>x.Description)).ForMember(x=>x.Code, opt => opt.MapFrom(x=>x.Code)).ForMember(x=>x.IsPrivate, opt => opt.MapFrom(x=>x.IsPrivate)).ForMember(x=>x.SubItems, opt => opt.MapFrom(x=>x.SubItems));
		}
	}
