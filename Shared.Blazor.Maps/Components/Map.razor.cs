...
namespace FisSst.BlazorMaps
{
    public partial class Map : ComponentBase
    {
        ...
        private const string initEvents = "initMapEvents";
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (!IsInitialized)
            {
                ...
                await InitEvents(MapReference);
                await this.JsRuntime.InvokeVoidAsync("onAfterMapInitialized");
                this.StateHasChanged();
            }
        }

        public async Task InitEvents(IJSObjectReference mapReference)
        {
            await this.JsRuntime.InvokeVoidAsync(initEvents, MapReference);
        }
        ...
        public async Task OnMoveEnd(Func<MouseEvent, Task> callback)
        {
            await this.MapEvented.OnClick(callback);
        }
